...
class CHAIR(object):
    ...
    def __init__(self, imids, coco_path, anchor_file=None):
        ...
        #read in synonyms
        synonyms_path = resolve_rel_path('data/synonyms.txt')
    ...
    def _load_generated_captions_into_evaluator(self, cap_file, is_split_sent=False):

        '''
        Meant to save time so imid_to_objects does not always need to be recomputed.
        '''
        #Read in captions        
        self.caps, imids, self.metrics = load_generated_captions(cap_file, self.anchor_file, is_split_sent=is_split_sent)
    ...
    def compute_chair(self, cap_file, is_split_sent=False):
    
        '''
        Given ground truth objects and generated captions, determine which sentences have hallucinated words.
        '''
    
        self._load_generated_captions_into_evaluator(cap_file, is_split_sent=is_split_sent)
        ...
        for i, cap_eval in enumerate(caps):
            ...
            cap_dict = {'image_id': cap_eval['image_id'], 
                        ...
                        'hallucination_idxs': [], 
                        'words': raw_words,
                        'mscoco_gt_words_detail': [],
                        'mscoco_generated_words_detail': [],
                        }
            ...
            for word, node_word, idx in zip(words, node_words, idxs):
                cap_dict['mscoco_generated_words_detail'].append((word, node_word, idx))
                if node_word not in gt_objects:
                    ...
                    cap_dict['mscoco_gt_words_detail'].append((word, node_word, idx))
...
def load_generated_captions(cap_file, anchor_file=None, is_split_sent=False):
    #Read in captions        
    caps = json.load(open(cap_file))
    ...
    if anchor_file is not None:
        ...
        for item in anchors:
            anchor_imgids.append(item['image_id'])
    ...
    try:
        ...
        for c in caps:
            if anchor_file is not None:
                if c['image_id'] not in anchor_imgids:
                    continue
            if is_split_sent:
                sents = sent_tokenize(c['answer'])
                for sent in sents:
                    items.append({
                        'image_id': c['image_id'],
                        'caption': sent,
...
def main(cap_file, annotation_path, anchor_file=None, is_split_sent=False):
    _, imids, _ = load_generated_captions(cap_file, anchor_file=anchor_file, is_split_sent=is_split_sent)
    # imids = [381173, 58569]

    # evaluator = CHAIR(imids, coco_path)
    
    evaluator = CHAIR(imids, annotation_path, anchor_file=anchor_file) 
    evaluator.get_annotations()
    cap_dict = evaluator.compute_chair(cap_file, is_split_sent=is_split_sent) 
    
    print_metrics(cap_dict)
    
    return cap_dict
    # save_hallucinated_words(cap_file, cap_dict)
 
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--cap_file", type=str, default='')
    parser.add_argument("--anchor_file", type=str, default=None)
    parser.add_argument("--annotation_path", type=str, default='coco/annotations')
    args = parser.parse_args()

    _, imids, _ = load_generated_captions(args.cap_file, anchor_file=args.anchor_file)
    # imids = [381173, 58569]

    # evaluator = CHAIR(imids, args.coco_path)
    
    evaluator = CHAIR(imids, args.annotation_path, anchor_file=args.anchor_file) 
    evaluator.get_annotations()
